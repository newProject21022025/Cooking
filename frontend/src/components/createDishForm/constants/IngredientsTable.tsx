...
import { units, ingredientsByCategory, mainCategories, optionalCategories } from "./ingredientsData";
import { FormValues, IngredientForm } from "@/types/dish";

interface Props {
  name: keyof FormValues; // тепер name точно вказує на поле форми
  label: string;
  type: "main" | "optional";
}
...
export default function IngredientsTable({ name, label, type }: Props) {
  ...
  const ingredients: IngredientForm[] = values[name] as IngredientForm[];
                    ...
                    <td>
                      ...
                      >
                        ...
                        {ingredientsByCategory[ing.category]?.map((item) => (
                          <option key={item.name_ua} value={item.name_ua}>{item.name_ua}</option>
                        ))}
                      </Field>
                    </td>
                    <td>
