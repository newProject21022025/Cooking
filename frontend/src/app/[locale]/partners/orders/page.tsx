...
export default function Orders() {
  ...
  useEffect(() => {
    ...
    const loadOrders = async () => {
      try {
        const partnerOrders: Order[] = await fetchOrdersByPartnerApi(partnerId);
        setOrders(partnerOrders);
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : String(err);
        setError(message || "Помилка при завантаженні замовлень");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleStatusChange = async (orderNumber: string, newStatus: string) => {
    try {
      const response: { order: Order } = await updateOrderStatusApi(orderNumber, newStatus);
      setOrders((prev) =>
        prev.map((o) => (o.orderNumber === orderNumber ? response.order : o))
      );
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      alert("Помилка оновлення статусу: " + message);
    }
  };
  ...
  const handleDelete = async (orderNumber: string) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err);
      alert("Помилка видалення замовлення: " + message);
    }
  };
